...
if(isset($_POST['doctorInsert'])) {
  ...
  $dfn=mysqli_real_escape_string($db,$_POST['dfn']);
  $dln=mysqli_real_escape_string($db,$_POST['dln']);
  $dcn=mysqli_real_escape_string($db,$_POST['dcn']);
...
if (empty($dcn)){ array_push($errors, "*Contact is required"); }
   ...
   //IF NOT EXIST IN ITS OWN DB  OR HQ DB THEN INSERT IN ITS OWN DB
	...
	if (count($errors) == 0) {
         $query = "INSERT INTO doctor(FirstName,LastName,contact) VALUES ('$dfn','$dln','$dcn');";	
      	 $result = mysqli_query($connakram, $query);
      	if($result){
			echo "Data inserted";
	  	}
		else{
